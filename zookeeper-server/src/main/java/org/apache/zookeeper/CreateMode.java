...
public enum CreateMode {
     ...
     * （译：客户端断开连接后，znode不会自动删除。）
     */
    PERSISTENT(0, false, false, false, false),
    /**
     * The znode will not be automatically deleted upon client's disconnect,
     * and its name will be appended with a monotonically increasing number.
     * （译：客户端断开连接后，znode不会自动删除，并且其名称将附加一个单调递增的数字。）
     */
    PERSISTENT_SEQUENTIAL(2, false, true, false, false),
    /**
     * The znode will be deleted upon the client's disconnect.
     * （译：客户端断开连接后，znode将被删除。）
     */
    EPHEMERAL(1, true, false, false, false),
    /**
     * The znode will be deleted upon the client's disconnect, and its name
     * will be appended with a monotonically increasing number.
     * （译：客户端断开连接后，znode将被删除，并且其名称将附加一个单调递增的数字。）
     */
     ...
     * （译：znode将是一个容器节点。容器节点是特殊用途的节点，可用于诸如leader，lock等配方。
     *      当删除容器的最后一个子节点时，容器将成为服务器将来要删除的候选对象。
     ...
     * （译：客户端断开连接后，znode不会自动删除。但是，如果尚未在给定的TTL范围内修改znode，则它将在没有子节点时将其删除。）
     */
